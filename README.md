...
At a high level, the log parser uses regular expressions to match various messages logged 
by Cassandra and extract any useful information they contain into separate fields.  
The regexes extract fields from the messages using standard Python capture groups. Additional 
transformations can be applied to each of the captured values, and then a dictionary 
containing the resulting values on each line is returned.
...
The `CassandraStore` class defined in `cassandra_store.py` uses the DataStax Python
Driver and provides an `insert_generic` method that takes the name of a table and a
dictionary of arbitrary fields and inserts them into the table. The only required 
...
To allow the storage of arbitrary data in the table, the table has a map for each common data 
type, including boolean (b_), date (d_), integer (i_), float (f_), string (s_), and list (l_).  

The insert_generic function will also detect any other fields that exist in the 
table and automatically use them if their name matches one of the keys in the dictionary.  
The type of any additional fields is expected to match the type of the corresponding 
value passed in the dictionary.  Key/value pairs in the dictionary that do not match any 
field name in the table are inserted into the appropriate map based on the 
type of the value.
...
The method uses asynchronous inserts with a callback to handle errors. This maximizes
throughput for ingesting logs but can sometimes overwhelm Cassandra and result in timeouts.
Future work on the project will introduce throttling to prevent this.
