...
class SystemLog:
    ...
    def __init__(self, files=None):
        ...
        for line in fi:
            for rule in self.line_rules:
                line_fields = rule(self, line, extra_fields=dict(log_file=fi.filename(), log_lineno=fi.filelineno()))
    ...
    def push_thread(self, thread_name, thread_id, fields):
        self.threads[(thread_name, thread_id)] = fields
        
    def pop_thread(self, thread_name, thread_id):
        if (thread_name, thread_id) in self.threads:
            fields = self.threads[(thread_name, thread_id)]
            del self.threads[(thread_name, thread_id)]
            return fields
        else:
            return {}
    ...
    def message_line(self, line_fields, extra_fields):
        ...
        line_fields['thread_id'] = None if line_fields['thread_id'] == '' else int(line_fields['thread_id'])
    ...
    @group(line_rules)
    @regex(r'(?P<exception>[^:]*): (?P<exception_message>.*)')
    ...
    def trace_line(self, line_fields, extra_fields):
        ...
        if 'caused_by' in self.line[-1]:
            self.lines[-1]['caused_by'][-1].get('stack_trace', []).append(line_fields)
        else:
            self.lines[-1].get('stack_trace', []).append(line_fields)
        
    @group(line_rules)
    @regex(r'Caused by: (?P<exception>[^:]*): (?P<exception_message>.*)')
    def caused_by_line(self, line_fields, extra_fields):
        'Parse nested exceptions'
        self.lines[-1].get('caused_by', []).append(line_fields)
    ...
    @group(message_rules['CassandraDaemon'], message_rules['StorageService'])
    ...
    def begin_flush(self, message_fields, line_fields):
        ...
        self.push_thread(line_fields['thread_name'], line_fields['thread_id'], message_fields)
    ...
    def end_flush(self, message_fields, line_fields):
        ...
        message_fields.update(self.pop_thread(line_fields['thread_name'], line_fields['thread_id']))
